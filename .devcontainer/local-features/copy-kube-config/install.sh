...
echo "alias kubectl='kubectl --context=kind-kind --insecure-skip-tls-verify'" | tee -a /root/.bashrc >> /root/.zshrc
if [ ! -z "${USERNAME}" ] && [ "${USERNAME}" != "root" ]; then
    echo "source /usr/local/share/copy-kube-config.sh" | tee -a /home/${USERNAME}/.bashrc >> /home/${USERNAME}/.zshrc
    echo "alias kubectl='kubectl --context=kind-kind --insecure-skip-tls-verify'" | tee -a /home/${USERNAME}/.bashrc >> /home/${USERNAME}/.zshrc
fi